...
void Can_Init( const Can_ConfigType* Config )
{
	...
	#endif
		{
			...
			//							CHECK THIS OUTSTANDING HABD !!! Also Micro_Registers.h ta7t 5ales
			uint8 i;
			for (i=0; i<96; i++)
			{
			...
			/* INTERRUPT INIT 				// Reham please check this part or tell me what to ask Moro !!!
...
Can_ReturnType Can_Write( Can_HwHandleType Hth, const Can_PduType* PduInfo )
{
...
#endif
	{
		...
		else
		{
            ...
            */
            uint8 sdu_data = *(PduInfo->sdu);
            uint32 MB31_ID = ((PduInfo->id)<<18);
            uint32 MB31_DATA_0 = (uint32)(sdu_data << 24);
            uint32 MB31_CS = ((PduInfo->length)<<16);

            printf ("\n\nMB31 ID = %d\n", MB31_ID);             // out: 40,000 in hex
            printf ("MB31 DATA0 = %d\n", MB31_DATA_0);      // out: 4,000,000 in hex
            printf ("MB31 CS = %d\n", MB31_CS);             // out: 80,000 in hex
...
void CanReceptionInterrupt(void)
{
	/* RECEIVE PROCESS */
    ...
    uint32 MB31_CS = 0x01080000;                      // CS word = 1, length = 8
    uint32 MB31_DATA_0 = 0x02000000;                  // Data = 1 decimal, 1 hex
    uint32 MB31_ID = 0x001C0000;                      // ID = 7 >> 1C

	// SEND & RECEIVE TRY
	MB31_CS = 0x01080000;                      // CS word = 1, length = 8
    MB31_DATA_0 = 0x02000000;                  // Data = 1 decimal, 1 hex
    MB31_ID = 0x001C0000;                      // ID = 7 >> 1C
