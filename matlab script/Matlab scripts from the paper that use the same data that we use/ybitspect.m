...
%% OPTIONS
frqthr=35;              %high frequency threshold {try 35,30,25,20,15,10}
...
sumch=false;            %average powers over all channels {try true/false}
TA=ceil(30/dT);         %average power over time {try values 0-60}

ntrain=[1,2,3,4,5];         %try different splits of train/test datasets
ntest=[5];

M=5;                   %number of copies

%% LOAD DATA
load ge_eegkaydi1.mat
O{1}=o;

load ge_eegkaydi2.mat
O{2}=o;

load ge_eegkaydi3.mat
O{3}=o;

load ge_eegkaydi4.mat
O{4}=o;

load ge_eegkaydi5.mat
O{5}=o;
...
for cnt=1:M
  ...
  for i=1:nft
      if(cnt<=length(ntrain))
          k=cnt;
      else
        k=randi(length(ntrain));
      end
      kk=ntrain(k);
...
fprintf('Confusion matrix, states vs class 0 1 2 3\n')
disp(CM)
